            ...
            for stmt in body.get("stmts", []):
                if "Assign" in stmt:
                    ...
                    if op in ["SubAssign", "AddAssign"]:
                        zasm_lines.append(f"PUSH {target_addr} ; Load {target} for compound assignment")
                    ...
                    if "Ident" in ret:
                        ...
                        zasm_lines.append(f"PUSH {ret_addr} ; Load return value {ret_name}")
                        zasm_lines.append("LDR")
                    elif "Literal" in ret:
                        lit_val = ret["Literal"]
                        zasm_lines.append(f"PUSH {lit_val}")
...
if __name__ == '__main__':
    ...
    print(f"[C5-REAL] Dumping Anvil AST via Cargo...")
