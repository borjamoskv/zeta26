        ...
        if char in ["+", "-", ">", "<"]:
            ...
            if char == ">":
                if count == 1:
                    ...
                    zeta_code += encode_number(count) + "A"
            elif char == "<":
                if count == 1:
                    zeta_code += "L"
                else:
                    zeta_code += encode_number(count) + "B"
            elif char == "+":
                if count == 1:
                    zeta_code += "HHVKIU"
                else:
                    zeta_code += "HHV" + encode_number(count) + "AIU"
            elif char == "-":
                if count == 1:
                    zeta_code += "HHVLIU"
                else:
                    zeta_code += "HHV" + encode_number(count) + "BIU"
        else:
            if char == ".":
                zeta_code += "HVM"
            elif char == ",":
...
if __name__ == "__main__":
    ...
    print(
        f"[C5-REAL] Brainfuck RLE+JIT-Transmuted to ZETA: {zeta_file} (Entropy Purged - length: {len(zeta)})"
    )
